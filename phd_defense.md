...
+  to simplify the proof of correctness of the Karp-Miller algorithm
+ to present the first simple modification of the original but incomplete Minimal Coverability Tree algorithm
+ to prove that the supplementary memory  needed by our algorithm is elementary (2-EXPSPACE)
+ to implement a prototype MinCov, showing experimentally that it is the most efficient one compared to other tools computing the Clover.
